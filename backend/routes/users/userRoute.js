...
router.route("/me/changes/:id ").post(upload.single("newAvatar"), async (req, res)=>{
     console.log(req.body);
    const {newName, newUsername, oldPassword, newPassword} = req.body
     const cookies = req.cookies.accessCookie
    if(!cookies) return res.sendStatus(204) // NO CONTENT
    const decodedUser = jwt.decode(cookies)
    ...
    if(req.file){
        const picture = await cloudinary.uploader.upload(req.file.path, {
            public_id: `profile_picture_${userToUpdate._id}`,
            folder: `avatar/${userToUpdate._id}`
  
           })
        
        const newPicture = picture.secure_url
        
        console.log("newPicture:", newPicture);

        // userToUpdate.profilePicture = newAvatar
        await User.updateOne({"_id": userToUpdate._id}, {$set: {"profilePicture": newPicture}})
